...
class DocumentLoader:
    ...
    def load_file(self, file_path: Path) -> list[Document]:
        ...
        try:
            ...
            enriched_documents = self._enrich_metadata(
                documents, file_path, file_type
            )
            ...
            logger.error("Failed to load file", extra={
                "file_path": str(file_path),
                "error": str(e)
            })
            raise
    ...
    def load_directory(self, directory_path: Path, recursive: bool = False) -> list[Document]:
        ...
        logger.info("Loading directory", extra={
        ...
        for file_path in supported_files:
            try:
                ...
                logger.warning("Skipping file due to error", extra={
                    "file_path": str(file_path),
                    "error": str(e)
                })
        
        logger.info("Directory loaded", extra={
    ...
    def load_file_iterator(
        ...
        logger.info("Loading file as iterator", extra={
            "file_path": str(file_path)
        })
